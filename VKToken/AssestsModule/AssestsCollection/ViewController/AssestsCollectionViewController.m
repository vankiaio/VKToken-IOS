...
//  VKToken
//
//  Created by vankiachain on 2018/10/18.
...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"资产归集", nil) rightBtnTitleName:NSLocalizedString(@"归集说明", nil) delegate:self];
...
- (void)configHeaderView{
    self.headerView.assestsNameLabel.text =  IsNilOrNull(self.currentToken) ? SymbolName_VKT : self.currentToken.token_symbol;
    self.headerView.contractNameLabel.text = [NSString stringWithFormat:@"(%@)", IsNilOrNull(self.currentToken) ? ContractName_VKTIOTOKEN : self.currentToken.contract_name];
}
