...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"发红包", nil) rightBtnTitleName:NSLocalizedString(@"红包记录", nil) delegate:self];
...
- (void)pushTransactionDidFinish:(TransactionResult *)result{
    ...
    if ([result.code isEqualToNumber:@0]) {
        NSLog(NSLocalizedString(@"转账到vktredpacket成功!", nil));
