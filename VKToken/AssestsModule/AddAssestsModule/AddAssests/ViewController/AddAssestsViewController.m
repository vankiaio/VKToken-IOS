...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"添加资产", nil)rightBtnImgName:@"search_icon" delegate:self];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell setAssestsSwitchStatusDidChangeBlock:^(RecommandToken *token, UISwitch *assestsSwitch) {
        ...
        if ([token.assetName isEqualToString:@"OCT"] || [token.assetName isEqualToString:@"VKT"]) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.assetName isEqualToString:@"OCT"] || [model.assetName isEqualToString:@"VKT"]) {
        [TOASTVIEW showWithText: NSLocalizedString(@"该资产无法取消关注", nil)];
    }
}
