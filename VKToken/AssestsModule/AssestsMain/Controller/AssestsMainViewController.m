...
//            [weakSelf.navigationController pushViewController:vc animated:YES];
    ...
    [self.navView setChangedBtn1DidClickBlock:^{
            ...
            vc.currentAssestsType = SymbolName_VKT;
            [weakSelf.navigationController pushViewController:vc animated:YES];
    ...
    [self.headerView setRedPacketBtnDidClickBlock:^{
            ...
            for (TokenInfo *token in weakSelf.get_token_info_service.dataSourceArray) {
                if ([token.token_symbol isEqualToString:@"OCT"] || [token.token_symbol isEqualToString:@"VKT"]) {
                    result = YES;
                }
            }
            if (result) {
                ...
                [TOASTVIEW showWithText: NSLocalizedString(@"请关注VKT或者OCT即可发送红包", nil)];
            }
...
////        model.url = @"https://coincreate.github.io/VKT_coincreate/coincreate_scatter.html";
//        model.applyName = @"VKT内存市场";
//        vc.model = model;
...
- (void)getAccount{
    WS(weakSelf);
    self.vktResourceService.getAccountRequest.name = CURRENT_ACCOUNT_NAME;
    [self.vktResourceService get_account:^(VKTResourceResult *result, BOOL isSuccess) {
        if (isSuccess) {
            [weakSelf.headerView updateViewWithVKTResourceResult:result];
        }
...
- (void)addCommonDialogHasTitleView{
    ...
    model.detail = NSLocalizedString(@"添加VKT账号继续操作", nil);
