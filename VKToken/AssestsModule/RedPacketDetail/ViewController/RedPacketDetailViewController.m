...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"红包", nil)rightBtnImgName:@"" delegate:self];
...
- (void)getRate{
    WS(weakSelf);
    if ([self.redPacketModel.coin isEqualToString:SymbolName_VKT]) {
        self.transferService.getRateRequest.coinmarket_id = @"vkt";
    }else if ([self.redPacketModel.coin isEqualToString:SymbolName_OCT]){
        self.transferService.getRateRequest.coinmarket_id = @"vankiachain";
    }else if ([self.redPacketModel.coin isEqualToString:SymbolName_CET]){
        self.transferService.getRateRequest.coinmarket_id = @"cet_vkt";
    }
