...
+ (NSData *)getBytesForSignature:(NSData *)chainId andParams:(NSDictionary *)paramsDic andCapacity:(int)capacity{
    VktByteWriter *writer = [[VktByteWriter alloc] initWithCapacity:capacity];
    if (chainId) {
    ...
    //transaction_extensions :: add transaction_extensions in transaction, although it dvkt not support yet.
    [writer putVariableUInt:0];
