...
#define JS_INTERACTION_METHOD_getVktAccount @"getVktAccount"
#define JS_INTERACTION_METHOD_getWalletWithAccount @"getWalletWithAccount"
#define JS_INTERACTION_METHOD_getVktBalance @"getVktBalance"
#define JS_INTERACTION_METHOD_getVktAccountInfo @"getVktAccountInfo"
...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:nil rightBtnTitleName:nil delegate:self];
...
- (WKWebView *)webView{
    if (!_webView) {
        ...
        if (@available(iOS 9.0, *)) {
            self.webView.customUserAgent = @"VKTokenIos";
        } else {
            // Fallback on earlier versions
        }
        self.allowZoom = YES;
...
-(void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation{
    self.allowZoom = NO;
    [self passVktAccountNameToJS];
    [self passWalletInfoToJS];
    WS(weakSelf);
    // 确保js 能收到 vktAccountName
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [weakSelf passVktAccountNameToJS];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([self.WKScriptMessageName isEqualToString:JS_INTERACTION_METHOD_PUSHACTION] || [self.WKScriptMessageName isEqualToString:JS_INTERACTION_METHOD_PUSH]) {
        ...
        if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getVktAccount]) {
            [self js_pushMessage_method_getVktAccount];
        ...
        }else if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getVktBalance]){
            [self js_pushMessage_method_getVktBalance];
        }else if ([self.dappPushMessageModel.methodName isEqualToString:JS_INTERACTION_METHOD_getVktAccountInfo]){
            [self js_pushMessage_method_getVktAccountInfo];
...
- (void)passVktAccountNameToJS{
    NSString *js = [NSString stringWithFormat:@"getVktAccount('%@')", self.choosedAccountName];
...
- (void)js_pushMessage_method_getVktAccount{
    WS(weakSelf);
    [self.dappDetailService getVktAccount:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}
...
- (void)js_pushMessage_method_getVktBalance{
    WS(weakSelf);
    GetVktBalanceModel *model = [GetVktBalanceModel mj_objectWithKeyValues:self.dappPushMessageModel.params];
    ...
    [self.dappDetailService getVktBalance:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}

- (void)js_pushMessage_method_getVktAccountInfo{
    WS(weakSelf);
    [self.dappDetailService getVktAccountInfo:^(id service, BOOL isSuccess) {
        [weakSelf responseToJsWithJSMethodName:JS_METHODNAME_CALLBACKRESULT SerialNumber:weakSelf.dappPushMessageModel.serialNumber andMessage:(NSString *)service];
    }];
}
...
- (void)dappChangeAccountOnNavigationRightViewDidClick{
    ...
    builder.cancelText = NSLocalizedString(@"选择您的VKT账号", nil);
    WS(weakSelf);
...
- (void)loadWebView{
    ...
    if ([self.model.dappUrl isEqualToString:@"https://vktflare.io"]) {
