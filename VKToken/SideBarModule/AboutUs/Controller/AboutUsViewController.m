...
@property(nonatomic , strong) Get_VKToken_info_request *get_VKToken_info_request;
@end
...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"关于我们", nil)rightBtnImgName:@"" delegate:self];
...
- (Get_VKToken_info_request *)get_VKToken_info_request{
    if (!_get_VKToken_info_request) {
        _get_VKToken_info_request = [[Get_VKToken_info_request alloc] init];
    }
    return _get_VKToken_info_request;
}
...
- (void)buildDataSource{
    WS(weakSelf);
    [self.get_VKToken_info_request getDataSusscess:^(id DAO, id data) {
        Get_VKToken_info_Result *result = [Get_VKToken_info_Result mj_objectWithKeyValues:data];
