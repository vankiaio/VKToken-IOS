...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"交易记录", nil)rightBtnImgName:@"" delegate:self];
...
- (void)requestTransactionHistory{
    ...
    self.mainService.getTransactionRecordsRequest.symbols = [NSMutableArray arrayWithObjects:@{@"symbolName":@"VKT"  , @"contractName": ContractName_VKTIOTOKEN }, @{@"symbolName":@"OCT"  , @"contractName": ContractName_OCTOTHEMOON },nil];
    [self loadNewData];
}
