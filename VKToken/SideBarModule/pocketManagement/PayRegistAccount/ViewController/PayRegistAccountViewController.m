...
@property(nonatomic , strong) VktPrivateKey *ownerPrivateKey;
@property(nonatomic , strong) VktPrivateKey *activePrivateKey;
...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"创建账号", nil)rightBtnTitleName:@"" delegate:self];
...
- (void)createAllKeys{
    self.ownerPrivateKey = [[VktPrivateKey alloc] initVktPrivateKey];
    self.activePrivateKey = [[VktPrivateKey alloc] initVktPrivateKey];
    NSLog(@"ownerPrivateKey:\n %@ \n ownerPublicKey:\n %@\n activePrivateKey:\n%@\n activePublicKey:\n %@\n", self.ownerPrivateKey.vktPrivateKey , self.ownerPrivateKey.vktPublicKey , self.activePrivateKey.vktPrivateKey, self.activePrivateKey.vktPublicKey);
}
...
- (void)configCreateAccountOrderRequestParams{
    ...
    if (self.headerView.privateKeyBeSameModeBtn.selected == YES) {
        self.payRegistAccountService.createAccountOrderRequest.ownerKey = self.ownerPrivateKey.vktPublicKey;
        self.payRegistAccountService.createAccountOrderRequest.activeKey = self.ownerPrivateKey.vktPublicKey;
    }else{
        self.payRegistAccountService.createAccountOrderRequest.ownerKey = self.ownerPrivateKey.vktPublicKey;
        self.payRegistAccountService.createAccountOrderRequest.activeKey = self.activePrivateKey.vktPublicKey;
    }
}
...
- (void)storeAccountInfoToLocalDatabase{
    ...
    if (self.headerView.privateKeyBeSameModeBtn.selected == YES) {
        model.account_owner_public_key = self.ownerPrivateKey.vktPublicKey;
        model.account_active_public_key = self.ownerPrivateKey.vktPublicKey;
        model.account_owner_private_key = [AESCrypt encrypt:self.ownerPrivateKey.vktPrivateKey password:self.password];
        model.account_active_private_key = [AESCrypt encrypt:self.ownerPrivateKey.vktPrivateKey password:self.password];
    }else{
        model.account_owner_public_key = self.ownerPrivateKey.vktPublicKey;
        model.account_active_public_key = self.activePrivateKey.vktPublicKey;
        model.account_owner_private_key = [AESCrypt encrypt:self.ownerPrivateKey.vktPrivateKey password:self.password];
        model.account_active_private_key = [AESCrypt encrypt:self.activePrivateKey.vktPrivateKey password:self.password];
    }
