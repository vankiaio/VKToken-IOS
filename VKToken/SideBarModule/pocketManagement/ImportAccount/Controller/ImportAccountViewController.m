...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"导入账号", nil)rightBtnImgName:@"scan_black" delegate:self];
...
- (void)validateInputFormat{
    ...
    private_active_Key_is_validate = [VKT_Key_Encode validateWif:self.headerView.private_activeKey_tf.text];
    private_owner_Key_is_validate = [VKT_Key_Encode validateWif:self.headerView.private_ownerKey_TF.text ];
    
    // VKT have only single key
//    if (self.headerView.agreeTermBtn.isSelected == YES) {
    if (YES) {
...
- (void)createPublicKeys{
    ...
    active_public_key_from_local = [VKT_Key_Encode vkt_publicKey_with_wif:self.headerView.private_activeKey_tf.text];
    owner_public_key_from_local = [VKT_Key_Encode vkt_publicKey_with_wif:self.headerView.private_ownerKey_TF.text];
    // 请求该账号的公钥
    WS(weakSelf);
...
- (void)importWithQRCodeBtnDidClick:(UIButton *)sender{
    ...
    if (device) {
        ...
        if (status == AVAuthorizationStatusNotDetermined) {
            ...
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"温馨提示", nil)message:NSLocalizedString(@"请去-> [设置 - 隐私 - 相机 - VKToken] 打开访问开关", nil)preferredStyle:(UIAlertControllerStyleAlert)];
