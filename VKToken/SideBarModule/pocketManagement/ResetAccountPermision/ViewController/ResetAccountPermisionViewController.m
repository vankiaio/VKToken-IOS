...
@property(nonatomic , strong) VktPrivateKey *privateKey;
@end
...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"变更权限", nil)rightBtnTitleName:@"" delegate:self];
...
- (VktPrivateKey *)privateKey{
    if (!_privateKey) {
        _privateKey = [[VktPrivateKey alloc] initVktPrivateKey];
    }
    return _privateKey;
}
...
- (void)validateInputFormat{
    // 验证私钥格式是否正确
    private_Key_is_validate = [VKT_Key_Encode validateWif:self.headerView.textView.text ];
    // 将用户导入的私钥生成公钥
    public_key_from_local = [VKT_Key_Encode vkt_publicKey_with_wif:self.headerView.textView.text];
...
- (void)pushActicon:(NSDictionary *)argsDict{
    self.abi_json_to_binRequest.code = ContractName_VKTIO;
    self.mainService.code = ContractName_VKTIO;
...
- (void)addGeneratePrivateKeyView{
    ...
    self.generatePrivateKeyView.contentTextView.text = self.privateKey.vktPrivateKey;
