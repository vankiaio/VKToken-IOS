//
//  VKTResourceManageViewController.m
//  VKToken
//
...
@interface VKTResourceManageViewController ()<PageSegmentViewDelegate, UINavigationControllerDelegate, CommonDialogHasTitleViewDelegate>
@property(nonatomic, strong) NavigationView *navView;
@property(nonatomic , strong) VKTResourceService *mainService;
...
@implementation VKTResourceManageViewController


- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"资源管理", nil) rightBtnImgName:@"" delegate:self];
...
- (VKTResourceService *)mainService{
    if (!_mainService) {
        _mainService = [[VKTResourceService alloc] init];
    }
    return _mainService;
}
...
- (void)buildDataSource{
    ...
    [self.mainService get_account:^(VKTResourceResult *result, BOOL isSuccess) {
        if (isSuccess) {
...
- (void)commonDialogHasTitleViewConfirmBtnDidClick:(UIButton *)sender{
    ...
    model.dappUrl = [NSString stringWithFormat:@"https://cpuemergency.com/embed_vankiachain.html"];
