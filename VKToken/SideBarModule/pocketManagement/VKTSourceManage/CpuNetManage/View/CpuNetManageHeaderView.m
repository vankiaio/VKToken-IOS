...
- (void)changeActionBaseViewLabelTap:(UITapGestureRecognizer *)sender{
    ...
    if (sender.view.tag == 1000) {
        ...
        self.cpuAmountInputTF.placeholder = NSLocalizedString(@"输入VKT数量", nil);
        self.netAmountInputTF.placeholder = NSLocalizedString(@"输入VKT数量", nil);
        self.cpuAmountUnitLabel.text = SymbolName_VKT;
        self.netAmountUnitLabel.text = SymbolName_VKT;
        [self.confirmBtn setTitle:NSLocalizedString(@"确认抵押", nil) forState:(UIControlStateNormal)];
        self.avalibleVKTAmountLabel.hidden = NO;
        ...
        self.avalibleVKTAmountLabel.hidden = YES;
    }
}
...
//        self.brrowStakeLabel.text = [NSString stringWithFormat:@"%.4f", totalCpuStr.doubleValue + totalNetStr.doubleValue];
    ...
    self.cpuBorrowLabel.text = [NSString stringWithFormat:@"%@ %.4f %@",  NSLocalizedString(@"借用", nil), totalCpuStr.doubleValue - selfCpuStr.doubleValue, SymbolName_VKT];
    ...
    self.netBorrowLabel.text = [NSString stringWithFormat:@"%@ %.4f %@",  NSLocalizedString(@"借用", nil), totalNetStr.doubleValue - selfNetStr.doubleValue, SymbolName_VKT];
    ...
    if (model.data.refund_request) {
        ...
        self.unstakeAmountLabel.text = [NSString stringWithFormat:@"%.4f VKT", model.data.refund_request_cpu_amount.doubleValue  +  model.data.refund_request_net_amount.doubleValue];
    }else{
        self.unstakeTimelabel.text = @"--/--";
        self.unstakeAmountLabel.text = [NSString stringWithFormat:@"0.0000 %@", SymbolName_VKT];
    }
    
    
    self.avalibleVKTAmountLabel.text = [NSString stringWithFormat:@"%@:%@", NSLocalizedString(@"可用", nil), model.data.core_liquid_balance];
