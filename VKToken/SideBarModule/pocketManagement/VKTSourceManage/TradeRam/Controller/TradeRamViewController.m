...
- (NavigationView *)navView{
    if (!_navView) {
        ...
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:title rightBtnTitleName:nil delegate:self];
...
- (void)buildDataSource{
    WS(weakSelf);
    self.get_table_rows_request.code = ContractName_VKTIO;
    self.get_table_rows_request.scope = ContractName_VKTIO;
    ...
    [self.get_table_rows_request postOuterDataSuccess:^(id DAO, id data) {
        ...
        if ((quote_balanceStr.length > 4) && (base_balanceStr.length > 4)) {
            ...
            if ([self.pageType isEqualToString: NSLocalizedString(@"buy_ram", nil)]) {
                weakSelf.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f VKT", self.accountResult.data.vkt_balance.doubleValue* weakSelf.headerView.modifyRamSlider.value];
                weakSelf.headerView.predictLabel.text = [NSString stringWithFormat:@"预计配额：%@ bytes", [[self.accountResult.data.vkt_balance yw_stringByDividingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:@"0.5" withRoundingMode:(NSRoundPlain) scale:4] ];
                
                
            }else if ([self.pageType isEqualToString: NSLocalizedString(@"sell_ram", nil)]){
                weakSelf.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f bytes", self.vktResourceResult.data.ram_max.doubleValue * weakSelf.headerView.modifyRamSlider.value  ];
                weakSelf.headerView.predictLabel.text = [NSString stringWithFormat:@"%@：%@ VKT", NSLocalizedString(@"预计出售价格", nil), [[self.vktResourceResult.data.ram_max yw_stringByMultiplyingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:@"0.5" withRoundingMode:(NSRoundPlain) scale:4] ];
...
- (void)modifySliderDidSlide:(UISlider *)sender{
    ...
    if ([self.pageType isEqualToString: NSLocalizedString(@"buy_ram", nil)]) {
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f VKT", self.accountResult.data.vkt_balance.doubleValue * progress];
    self.headerView.predictLabel.text =  [NSString stringWithFormat:@"%@ bytes", [[self.accountResult.data.vkt_balance yw_stringByDividingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:[NSString stringWithFormat:@"%.2f", progress] withRoundingMode:(NSRoundPlain) scale:4]];
        
    }else if ([self.pageType isEqualToString: NSLocalizedString(@"sell_ram", nil)]){
        self.headerView.amountLabel.text = [NSString stringWithFormat:@"%.4f bytes", self.vktResourceResult.data.ram_max.doubleValue * progress  ];
        self.headerView.predictLabel.text = [NSString stringWithFormat:@"预计出售价格：%@ VKT", [[self.vktResourceResult.data.ram_max yw_stringByMultiplyingBy:self.price_str withRoundingMode:(NSRoundPlain) scale:4] yw_stringByMultiplyingBy:[NSString stringWithFormat:@"%.2f", progress] withRoundingMode:(NSRoundPlain) scale:4]];
    }
}
...
- (void)buyRam{
    ...
    self.buy_ram_abi_json_to_bin_request.code = ContractName_VKTIO;
    self.buy_ram_abi_json_to_bin_request.payer = self.vktResourceResult.data.account_name;
    self.buy_ram_abi_json_to_bin_request.receiver = self.vktResourceResult.data.account_name;
...
- (void)sellRam{
    ...
    self.sell_ram_abi_json_to_bin_request.code = ContractName_VKTIO;
    self.sell_ram_abi_json_to_bin_request.account = self.vktResourceResult.data.account_name;
