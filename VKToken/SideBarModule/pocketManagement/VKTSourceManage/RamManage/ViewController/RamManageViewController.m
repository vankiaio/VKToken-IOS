...
- (void)buildDataSource{
    ...
    [self.getCurrentRamPriceRequest getDataSusscess:^(id DAO, id data) {
        ...
        if (result.data.count > 0) {
            ...
            weakSelf.headerView.nowPriceLanel.text = [NSString stringWithFormat:@"%.4fVKT/KB", model.price.doubleValue];
        }
...
- (void)ramManageHeaderViewConfirmStakeBtnDidClick{
    ...
    if (IsStrEmpty(self.headerView.vktAmountTF.text)) {
    ...
    if (self.headerView.ramManageHeaderViewCurrentAction == RamManageHeaderViewCurrentActionBuyRam) {
        if (self.headerView.vktAmountTF.text.doubleValue > self.vktResourceResult.data.core_liquid_balance.doubleValue ) {
...
- (void)buyRam{
    ...
    self.buy_ram_abi_json_to_bin_request.quant = [NSString stringWithFormat:@"%.4f %@",self.headerView.vktAmountTF.text.doubleValue, SymbolName_VKT];
    WS(weakSelf);
...
- (void)sellRam{
    ...
    double bytes;
    bytes = self.headerView.vktAmountTF.text.doubleValue * 1024;
...
- (void)pushTransactionDidFinish:(VKTResourceResult *)result{
    ...
    [self removeLoginPasswordView];
    [self loadNewData];
    self.headerView.vktAmountTF.text = nil;
}
