...
- (NavigationView *)navView{
    if (!_navView) {
        _navView = [NavigationView navigationViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NAVIGATIONBAR_HEIGHT) LeftBtnImgName:@"icon_back" title:NSLocalizedString(@"VIP创建", nil)rightBtnTitleName:@"" delegate:self];
...
- (void)createkeys{
    ...
    weakSelf.vipRegistAccountService.inviteCodeRegisterRequest.vktAccountName = weakSelf.headerView.accountNameTF.text;
    weakSelf.vipRegistAccountService.inviteCodeRegisterRequest.ownerKey = ownerPrivateKey.vktPublicKey;
    weakSelf.vipRegistAccountService.inviteCodeRegisterRequest.activeKey = activePrivateKey.vktPublicKey;
    ...
    [weakSelf.vipRegistAccountService createVKTAccount:^(id service, BOOL isSuccess) {
        ...
        if (isSuccess) {
            ...
            if ([code isEqualToNumber:@0]) {
                ...
                model.account_active_public_key = activePrivateKey.vktPublicKey;
                model.account_owner_public_key = ownerPrivateKey.vktPublicKey;
