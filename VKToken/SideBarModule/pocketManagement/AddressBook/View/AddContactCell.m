...
@interface AddContactCell() <UITextFieldDelegate>
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    if ([_delegate respondsToSelector:@selector(addContactCell:valueChange:)]) {
        [_delegate addContactCell:self valueChange:textField.text];
    }
}
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        label.textColor = [UIColor colorWithRGBHex:0x333333];
        _titleLabel = label;
    }
    return _titleLabel;
}
...
- (UITextField *)inputTextField {
    if (!_inputTextField) {
        ...
        textField.textColor = [UIColor colorWithRGBHex:0x333333];
